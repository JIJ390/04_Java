...
public class ThrowsService {
	 ...
	 * 
	 * 2) 예외 처리 구문을 한 곳에 묶어서 처리하기 위하여
	 * -> 실질적으로 예외를 처리하는 
	 * 	  try - catch 구문이 사방에 흩어져 있을 경우
	 * 	  돌려 받는 최종 결과가 
	 *    정상 결과인지, 예외처리된 결과인지
	 *    구분이 어려운 경우가 발생
	 *    
	 *    => 곳곳에서 발생하는 예외를 모아 한 번에 처리
	 * 
	 */
...
//		1) Exception 강제 발생
	...
	public void test3() throws IOException  {
		test4(1);	// 전달 인자 값 1 or 2
	}
	
	/**
	 * throws IOException : 경고 목적
	 * 
	 * 전달 받은 num 값에 따라 IOException 이 발생할 가능성이 있으므로
	 * 해당 메서드를 호출하는 곳에서는
	 * IOException 에 대한 예외 처리 구문을 작성해야만 한다
	 * 
	 * @param num
	 * @throws IOException 
	 */
		...
		do {
			...
			try {
				...
				input = -1;  	// 첫 반복 시 반복문이 종료되지 않도록 input 값을 변경
								// 문자열 입력시 입력을 받지 못해 기본값 0이 들어가 있음
				...
				System.out.println("발생한 예외를 test5() 에서 처리");
				e.printStackTrace();
...
//		num2 가 0 인 경우 
//		ArithmeticException (산술적 예외) 발생 가능성
//		UncheckedException 이라서 빨간 줄 X
		
//		ArithmeticException 발생하면 
//		호출한 곳으로 예외를 던지는 throws 사용
		
//		*** UncheckedException 은 
//		 	아무런 예외처리가 안되어 있는 경우
