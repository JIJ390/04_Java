...
//		   (컴파일러가 new Integer(num1) 형태로 변환)
		...
		int num2 = wrap1.intValue(); // Integer 의 값을 int 로 반환
		
		int num3 = wrap1;	// (Integer)
//		-> 컴파일러가 Auto Unboxing 을 수행
...
//		-> 제네릭에 기본 자료형 작성 시 오류 발생
		...
		list.add(300);
		list.add(1, 200); // 1 번 인덱스에 200을 추가 중간 삽입
...
//		(int)     4000 -> (long) 4000L  자동 형변환
	 ...
	 * --> 클래스명.필드명 / 클래스명.메서드 명() 사용 가능!!!
	 *    * (생성하지 않아도)
	 */
	public void test2() {
//		정수형 / 실수형 공통 필드
		System.out.println("byte 의 용량   : " + Byte.BYTES    + " 바이트");
		System.out.println("int  의 용량   : " + Integer.BYTES + " 바이트");
		System.out.println("double 의 용량 : " + Double.BYTES  + " 바이트");
		
		System.out.println("byte 의 용량   : " + Byte.SIZE     + " 비트");
...
//		다른 언어(특히 HTML, JS) 에서 전달 값이 있을 경우
//		보통 String 형태로 전달 되어져서 오는 경우가 많다
//		이를 Java 에서 기본 자료형으로 변환하는 경우 사용
//		단 Char 는 존재하지 않음
		...
		float f  = Float.parseFloat("5.1");
		double d = Double.parseDouble("6.2");
